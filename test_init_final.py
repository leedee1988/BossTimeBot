...
async def on_message(msg):
	...
	if client.get_channel(channel) != msg.channel :
		...
		if message.content.startswith('/뽑기'):
			ladder = []
			#ladder = message.content[5:].split(" ")
			ladder = message.content.split(" ")
			num_cong = int(ladder[0][-1])
			del(ladder[0])
			if num_cong <= len(ladder):
				result_ladder = random.sample(ladder, num_cong)
				print (result_ladder)
		...
		if message.content.startswith('!명치') or message.content.startswith('/정신차려'):
			await dbSave()
		...
		if message.content.startswith('!소환') or message.content.startswith('/소환'):
		...
		if message.content.startswith('!초기화') or message.content.startswith('/보탐 초기화'):
			basicSetting = []
			bossData = []
		...
		if message.content.startswith('보스탐') or message.content.startswith('/보스') or message.content.startswith('/보탐'):
			for i in range(bossNum):
				for j in range(bossNum):
					if bossTimeString[i] and bossTimeString[j] != '99:99:99':
						if bossTimeString[i] == bossTimeString[j] and i != j:
